(() => {
  ...
  const state = {
    ...
    savedOnly: false,
    searchIndex: -1,
    uiScale: storage.get("dana.uiScale", 1)
  };
  ...
  function cover(a){ return a?.cms && a.image ? a.image : `assets/covers/${a.slug}.svg`; }
  ...
  function applyPreferences(){
    ...
    $$(`[data-current-year]`).forEach(el=>el.textContent=String(currentYear));
    updateSavedUI();
  }
  ...
  function renderHome(options={}){
    ...
    const marquee=["NEWS","REPORTS","ESSAYS","FIELD NOTES","DATA","MEDIA","TECHNOLOGY","SOCIETY","دانه بالجهر","JEDDAH",String(currentYear)];

    main.innerHTML=`
      <section class="hero" id="home">
        <div class="shell hero-grid">
          <div class="hero-main reveal">
            <div class="hero-topline"><span class="edition-badge"><i></i> EDITORIAL PORTFOLIO · ${currentYear}</span><p>${esc(t("heroCopy"))}</p></div>
          ...
          <aside class="hero-side reveal">
            <div class="hero-side-head"><strong>${esc(t("latestArchive"))}</strong><span class="micro-label">NOW / ${currentYear}</span></div>
      ...
      <section class="section" id="about">
        <div class="shell">
          <div class="about-layout">
            <div class="about-sticky reveal"><span class="micro-label">03 / ABOUT</span><h2>${esc(t("aboutTitle"))}</h2><p>${esc(t("aboutLead"))}</p><p>${esc(t("aboutBody"))}</p><div class="about-signature"><img src="assets/brand/dana-mark.png" alt=""><span><strong>${esc(t("name"))}</strong><small>${esc(t("portfolio"))}</small></span></div></div>
    ...
    if(options.anchor) setTimeout(()=>scrollToSection(options.anchor),80); else if(!options.preserveScroll) scrollTo({top:0,behavior:"auto"});
  }
  ...
  function renderArticle(a,options={}){
    ...
    main.innerHTML=`<article class="article-page" data-current-article="${esc(a.slug)}">
      ...
      <div class="article-body-shell">
        <aside class="article-aside"><div class="aside-block"><span>${esc(t("article"))}</span><strong class="arabic-content" lang="ar" dir="rtl">${esc(categoryLabel(a))}</strong></div><div class="aside-block"><span>${esc(t("font"))}</span><div class="reading-controls"><button type="button" data-font="minus" aria-label="تصغير الواجهة">A−</button><button type="button" data-font-reset class="scale-reset" aria-label="الحجم الطبيعي"><span data-scale-value>${Math.round(state.uiScale*100)}%</span></button><button type="button" data-font="plus" aria-label="تكبير الواجهة">A+</button></div></div><div class="aside-block"><span>${esc(t("width"))}</span><div class="reading-controls"><button type="button" data-width="narrow">↔</button><button type="button" data-width="wide">⇔</button></div></div><div class="aside-block"><button type="button" data-share>${esc(t("share"))} ↗</button><button type="button" data-save="${esc(a.slug)}">${isSaved(a.slug)?esc(t("savedDone")):esc(t("save"))} ${isSaved(a.slug)?"★":"☆"}</button></div></aside>
    ...
    readingDock.innerHTML=`<button type="button" data-dock-font="minus" aria-label="تصغير الواجهة">A−</button><button type="button" data-dock-font-reset class="scale-reset" aria-label="الحجم الطبيعي"><span data-scale-value>${Math.round(state.uiScale*100)}%</span></button><button type="button" data-dock-font="plus" aria-label="تكبير الواجهة">A+</button><div class="dock-progress"><i></i></div><button type="button" data-dock-save="${esc(a.slug)}">${isSaved(a.slug)?"★":"☆"}</button><button type="button" data-focus>${esc(t("focus"))}</button>`;
  ...
  function bindArticleControls(a){
    ...
    $$('[data-font-reset],[data-dock-font-reset]').forEach(b=>b.addEventListener("click",resetFont));
  ...
  function applyUiScale(){
    const scale=Math.min(1.4,Math.max(.9,Number(state.uiScale)||1));
    state.uiScale=scale;
    document.documentElement.style.setProperty("--ui-scale",String(scale));
    document.body.style.zoom=String(scale);
    $$('[data-scale-value]').forEach(el=>el.textContent=`${Math.round(scale*100)}%`);
  }
  function changeFont(delta){
    const next=Math.round(Math.min(1.4,Math.max(.9,(Number(state.uiScale)||1)+(delta*.1)))*10)/10;
    state.uiScale=next;storage.set("dana.uiScale",next);applyUiScale();onScroll();
  }
  function resetFont(){ state.uiScale=1;storage.set("dana.uiScale",1);applyUiScale();onScroll(); }
  ...
  function openArchive(cat="all"){ if(cat==="stats")cat="all";state.savedOnly=cat==="saved";state.archiveCategory=["news","report","article","training"].includes(cat)?cat:"all";state.visible=9;scrollToSection("archive");setTimeout(renderArchive,250); }
  ...
  function bindGlobal(){
    ...
    ribbonTrack.addEventListener("click",e=>{
      const b=e.target.closest("[data-ribbon-action]");if(!b)return;
      e.preventDefault();
      const action=b.dataset.ribbonAction,target=b.dataset.target;
      const inArticle=new URLSearchParams(location.search).has("article");
      if(["news","report","article"].includes(action))state.workCategory=action;
      if(action==="archive"){state.savedOnly=false;state.archiveCategory="all";}
      if(inArticle){navigateHome(target);return;}
      if(["news","report","article"].includes(action))renderWorkConsole();
      requestAnimationFrame(()=>scrollToSection(target));
      if(action==="archive")setTimeout(renderArchive,180);
    });
